...
// ─── Subscription Store ──────────────────────────────────────
// Premium/monetization infrastructure — ready for future
// Google Play Billing, AdMob, Stripe integration.
...
const TIER_FEATURES: Record<SubscriptionTier, SubscriptionFeatures> = {
  free: { adFreeRooms: false, customThemes: false, prioritySupport: false, maxRoomSize: 5 },
  premium: { adFreeRooms: true, customThemes: true, prioritySupport: false, maxRoomSize: 15 },
  vip: { adFreeRooms: true, customThemes: true, prioritySupport: true, maxRoomSize: 50 },
};

export const useSubscriptionStore = create<SubscriptionState>((set) => ({
  tier: (localStorage.getItem('anisync_tier') as SubscriptionTier) || 'free',
  isPremium: localStorage.getItem('anisync_tier') !== 'free' && !!localStorage.getItem('anisync_tier'),
  showAds: !localStorage.getItem('anisync_tier') || localStorage.getItem('anisync_tier') === 'free',
  features: TIER_FEATURES[(localStorage.getItem('anisync_tier') as SubscriptionTier) || 'free'],
  setTier: (tier) => {
    localStorage.setItem('anisync_tier', tier);
    set({
      tier,
      isPremium: tier !== 'free',
      showAds: tier === 'free',
