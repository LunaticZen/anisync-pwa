...
app.get('/app', (_req, res) => serveSpaHtml(res));
app.get('/app/*', (_req, res) => serveSpaHtml(res));
...
// ── Access Code Gate ──
const SERVER_ACCESS_CODE = process.env.ACCESS_CODE || 'ani2026';

io.use((socket, next) => {
  const code = socket.handshake.auth?.accessCode;
  if (!code || code !== SERVER_ACCESS_CODE) {
    console.log(`[WS] Rejected: invalid access code from ${socket.handshake.address}`);
