...
function registerRoomHandlers(socket: TypedSocket) {
  ...
  socket.on('room:join', async (data, callback) => {
    try {
      ...
      if (result.status === 'pending') {
        socket.to(result.roomId).emit('room:join-request', {
        ...
        callback({ success: true, status: 'pending', roomId: result.roomId, hostId: result.hostId, roomName: result.roomName });
        return;
      }
  ...
  socket.on('room:cancel-request', (data) => {
    try {
      ...
      io!.to(data.roomId).emit('room:request-cancelled', { userId: socket.data.userId });
    } catch (err) {
