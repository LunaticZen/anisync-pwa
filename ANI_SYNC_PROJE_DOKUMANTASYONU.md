...
5. **Vidmoly (ve diğer) Frame-Buster'ların Android WebView'da Yanlış Engellenmeye Çalışılması (404 Tuzağı)**
   * **Neden Bozdum?** Kullanıcı, Turkanime'de bozuk bir Vidmoly videosuna tıkladığında tüm ekranın yeşil bir "404" vidmoly hata sayfasıyla (video olarak oynayan bir ekran) kaplandığını, yani asıl sayfanın Vidmoly'ye yönlendiğini bildirdi. Bunun `TurkanimeAdapter`'ın `extractIframe` fonksiyonu yüzünden olduğunu sandım ve bu özelliği kapattım. Ancak sorun çözülmedi. Ardından sorunun Vidmoly'nin kendi içindeki bir *Frame-Buster* (`if (top !== self) top.location.href = ...`) kodu olduğunu fark ettim. Bunu engellemek için Java tarafında `shouldOverrideUrlLoading` metoduna `if (currentUrl.contains("turkanime") && isVideoProvider(url)) return true;` ekledim.
   * **Ne Oldu?** Kullanıcı sorunun hala devam ettiğini söyledi. Çünkü Android WebView'da JavaScript kaynaklı yönlendirmeler (`location.href = ...` veya `location.replace()`) her zaman `shouldOverrideUrlLoading` fonksiyonunu **TETİKLEMEZ!** Bu metod genellikle yalnızca kullanıcı tarafından tıklanan bağlantılar (user-initiated) için güvenilirdir. Bu yüzden eklediğim Java katmanı koruması bypass edildi ve uygulama yine Vidmoly'ye yönlendi.
   * **Yapılması Gereken Çözüm:** Frame-Buster engellemenin en kesin yolu, WebView Java katmanı değil, doğrudan sayfanın kendisine (DOM'a) müdahale etmektir. `index.ts` veya Java üzerinden çalıştırılan bir `MutationObserver` ile sayfaya eklenen tüm `iframe` elementleri yakalanmalı ve onlara `sandbox="allow-scripts allow-same-origin allow-forms allow-popups"` niteliği (attribute) eklenmelidir. (Özellikle `allow-top-navigation` EKLENMEMELİDİR). Bu sayede iframe'in üst pencereyi (top window) yönlendirmesi tarayıcı tarafından donanımsal düzeyde kesin olarak reddedilir.

6. **Mobil Uygulama URL'sinin Sabitlenmesi (Render Domain'inden Kurtulma)**
