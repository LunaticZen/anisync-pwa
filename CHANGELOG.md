...
- **Klavye Tetikleme Sorunu (Xiaomi / Android):** Klavyenin kapalı olduğu durumlarda emoji butonuna basıldığında bazı telefonlarda (özellikle Xiaomi) klavyenin gereksiz yere açılması sorunu `onTouchStart` ile tıklama olaylarının sızması engellenerek çözüldü.
- **Android Klavye Görünürlüğü (Edge-to-Edge):** Android'de klavye açıldığında uygulamanın klavyenin altında kalmasını önlemek için resmi belgelere uygun olarak `windowSoftInputMode="adjustResize"` eklendi ve güvenli alan insets (ime) dinleyicilerinin düzgün çalışması sağlandı.
